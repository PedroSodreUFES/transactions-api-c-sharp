...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        CreateHeaderWithProfilePhtoAndName(page);
        ...
        foreach (var expense in expenses)
        {
            ...
            row.Cells[1].AddParagraph(expense.Date.ToString("HH:mm:ss"));
            SetStyleBaseForExpenseInformation(row.Cells[1]);

            row.Cells[2].AddParagraph(expense.Type.PaymentTypeToString());
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Pedro Sodré";
    ...
    private void CreateHeaderWithProfilePhtoAndName(Section page)
    {
        ...
        var pathFile = Path.Combine(directoryName!, "Logo", "api_foto.jpg");

        row.Cells[0].AddImage(pathFile);

        row.Cells[1].AddParagraph("Hey, Pedro Sodré");
