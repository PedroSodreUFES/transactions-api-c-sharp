...
public class UpdateExpenseUseCaseTest
{
    ...
    public async Task Success()
    {
        ...
        expense.Type.Should().Be((CashFlow.Domain.Enums.PaymentType)request.Type);
    }
    ...
    public async Task Error_Expense_Not_Found()
    {
        ...
        var act = async () => await useCase.Execute(id: 1001, request);

        var result = await act.Should().ThrowAsync<NotFoundException>();

        result.Where(ex =>
            ex.GetErrors().Count == 1 &&
            ex.GetErrors().Contains(ResourceErrorsMessage.EXPENSE_NOT_FOUND)
        );
    }
